    ...
    value: {
      ...
      observer: function(t, e) {
        var i = this.properties.min,
          ...
          r = null == t || isNaN(Number(t)) || t < i || t > a;
        this.setData({
          sliderValue: t,
          showHandle: !r
        }), r || this.updateSliderPosition()
      }
    },
    min: {
      type: Number,
  ...
  methods: {
    ...
    valueContent: function(t) {
      ...
      if (this.data.valueArray) {
        var i = Math.round((t - this.data.min) / (this.data.max - this.data.min) * (this.data.valueArray.length - 1));
        return this.data.valueArray[i] || ""
      }
    ...
    updateSliderPosition: function() {
      ...
      e.select(".slider-container").boundingClientRect(), e.exec((function(e) {
        if (e && e[0]) {
          var i = e[0].width,
            a = e[0].left,
            r = (t.data.sliderValue - t.properties.min) / (t.properties.max - t.properties.min),
            s = i - 24,
            n = r * s,
            o = 24 + (i - 24) * r,
    ...
    onSliderTouchStart: function(t) {
      this.properties.disabled ? this.triggerEvent("onDisableClick", {}) : (this.setData({
        showHandle: !0
      }), this.properties.disablePageScroll || wx.pageScrollTo({
        scrollTop: wx.getWindowInfo().windowHeight,
        duration: 0
      }), this.setData({
        moving: !0
      }))
    ...
    onSliderTouchMove: function(t) {
      if (!this.properties.disabled && this.data.moving) {
        ...
        var r = 24 + (this.data.containerWidth - 24) * e,
          s = this.valueContent(a);
        this.setData({
          sliderProgressWidth: r,
          sliderValue: a,
    ...
    onSliderTouchEnd: function(t) {
      !this.properties.disabled && this.data.moving && (t.stopPropagation && t.stopPropagation(), this.properties.disablePageScroll || setTimeout((function() {
        wx.pageScrollTo({
          scrollTop: wx.getWindowInfo().windowHeight,
          duration: 0
        })
    ...
    onSliderTap: function(t) {
      var e = this;
      if (this.properties.disabled) this.triggerEvent("onDisableClick", {});
      else {
        this.setData({
          showHandle: !0
        }), t.stopPropagation && t.stopPropagation(), t.preventDefault && t.preventDefault();
        ...
        i.select(".slider-container").boundingClientRect(), i.exec((function(i) {
          if (i && i[0]) {
            ...
            var n = Math.round(s * (e.properties.max - e.properties.min) + e.properties.min),
              o = Math.round(n / e.properties.step) * e.properties.step;
            s = (o - e.properties.min) / (e.properties.max - e.properties.min);
            var l = 24 + (e.data.containerWidth - 24) * s,
              d = e.valueContent(o);
            e.setData({
